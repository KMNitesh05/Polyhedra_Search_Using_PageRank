...
makePolyFreqs<-function(polyfile)
{
    ...
    for(i in sort(unique(polys$shsz[polys$shsz>3 & polys$shsz<12])))
    {
        ...
        polysseen=as.data.frame((table((polys$polynm[polys$shsz==i]))))
        polysseen1 = as.factor(polysseen)
        polysseen2 = droplevels(polysseen1)
        print(polysseen2)
...
#analyzePD('watercl1.input.pdfs.csv')
#makePolyFreqs('watercl.Hs4.Polys')
#writeformatted(10,'new')
makePolyFreqs('new.Polys')
